...
/**
 * This class defines HDF5 datatype characteristics and APIs for a data type. This class provides several methods to
 * convert an HDF5 datatype identifier to a datatype object, and vice versa. A datatype object is described by four
 * basic fields: datatype class, size, byte order, and sign, while an HDF5 datatype is presented by a datatype
 * identifier.
 *
...
public class H5Datatype extends Datatype
{
    ...
    /**
     * Constructs an named HDF5 data type object for a given file, dataset name and group path. The datatype object
     * represents an existing named datatype in file. For example,
     *
     * <pre>
    ...
    public H5Datatype(FileFormat theFile, String theName, String thePath, long[] oid) {
        ...
        if (theFile != null) {
            ...
            try {
                objInfo = H5.H5Oget_info_by_name(theFile.getFID(), this.getFullName(), HDF5Constants.H5O_INFO_BASIC,
    ...
    /**
     * Constructs a Datatype with specified class, size, byte order and sign. The following is a list of a few examples
     * of H5Datatype.
     * <ol>
     ...
     * @param tsize
     *               the size of the datatype in bytes, e.g. for a 32-bit integer, the size is 4. Valid values are
     *               NATIVE or a positive value. For string datatypes, -1 is also a valid value (to create a
     *               variable-length string).
     * @param torder
     *               the byte order of the datatype. Valid values are ORDER_LE, ORDER_BE, ORDER_VAX, ORDER_NONE and
     *               NATIVE.
     * @param tsign
    ...
    /**
     * Constructs a Datatype with a given native datatype identifier. For example, if the datatype identifier is a
     * 32-bit unsigned integer created from HDF5,
     *
     * <pre>
    ...
    /**
     * Converts values in an Enumeration Datatype to names. This method searches the identified enumeration datatype for
     * the values appearing in <code>inValues</code> and returns the names corresponding to those values. If a given
     * value is not found in the enumeration datatype, the name corresponding to that value will be set to
     * <code>"ENUM ERR value"</code> in the string array that is returned. If the method fails in general, null will be
     * returned instead of a String array. An empty <code>inValues</code> parameter would cause general failure.
     *
     * @param inValues
     ...
     */
    public String[] convertEnumValueToName(Object inValues)
            throws HDF5Exception {
    ...
    /**
     * Converts names in an Enumeration Datatype to values. This method searches the identified enumeration datatype for
     * the names appearing in <code>inValues</code> and returns the values corresponding to those names.
     *
     * @param in
     ...
     *                       If there is an error at the HDF5 library level.
     */
    public Object[] convertEnumNameToValue(String[] in)
            throws HDF5Exception {
        int size = 0;
    ...
    public byte[] convertBigDecimalToByte(BigDecimal num) {
        ...
        int scale = num.scale();
        byte[] bscale = new byte[] { (byte) (scale >>> 24), (byte) (scale >>> 16), (byte) (scale >>> 8),
                (byte) (scale) };
    ...
    public void fromNative(long tid) {
        ...
        if (tid < 0) {
            ...
            try {
                ...
                isVLEN = false;
                log.trace("fromNative(): tclass={}, tsize={}, torder={}, isVLEN={}", nativeClass, tsize, torder,
                        isVLEN);
            ...
            try {
                ...
                isChar = (H5.H5Tequal(tid, HDF5Constants.H5T_NATIVE_CHAR)
                        || isUchar);
                log.trace("fromNative(): tclass={}, tsize={}, torder={}, isUchar={}, isChar={}", nativeClass, tsize,
                        torder, isUchar, isChar);
            }
            catch (Exception ex) {
            ...
            if (IsAtomic
                    || (nativeClass == HDF5Constants.H5T_COMPOUND)) {
                try {
            ...
            if (IsAtomic
                    && !datatypeClassIsOpaque(nativeClass)) {
                try {
            ...
            log.trace("fromNative(): isUchar={}, nativePrecision={}, nativeOffset={}, nativePadLSB={}, nativePadMSB={}",
            ...
            if (nativeClass == HDF5Constants.H5T_ARRAY) {
                ...
                try {
                    isVLEN = H5.H5Tdetect_class(tid, HDF5Constants.H5T_VLEN)
                            || isVariableStr;
            ...
            try {
                ...
                for (int i = 0; i < compoundMemberTypes.size(); i++) {
                    ...
                    try {
                        ...
                        log.trace("createNative(): {} member[{}] with offset={} ID={}: ", memberName, i, memberOffset,
                                memberID);
        ...
        case CLASS_FLOAT:
            try {
                if (datatypeSize > 8)
                    ...
                    tid = H5.H5Tcopy(
                            (datatypeSize == 8) ? HDF5Constants.H5T_NATIVE_DOUBLE : HDF5Constants.H5T_NATIVE_FLOAT);
        ...
        case CLASS_STRING:
            try {
                ...
                H5.H5Tset_size(tid, (isVLEN
                        || datatypeSize < 0) ? HDF5Constants.H5T_VARIABLE : datatypeSize);
    ...
    public static final Object allocateArray(final H5Datatype dtype, int numPoints)
        ...
        if (dtype.isVarStr()) {
            ...
            default:
                ...
                || (typeClass == HDF5Constants.H5T_REFERENCE)) {
        ...
        else if ((typeClass == HDF5Constants.H5T_OPAQUE)
                || (typeClass == HDF5Constants.H5T_BITFIELD)) {
        ...
        if (tid >= 0) {
            try {
                ...
                if (tclass != HDF5Constants.H5T_FLOAT
                        && tclass != HDF5Constants.H5T_STRING && tclass != HDF5Constants.H5T_REFERENCE
                        && tclass != HDF5Constants.H5T_BITFIELD && tclass != HDF5Constants.H5T_OPAQUE
                        && tclass != HDF5Constants.H5T_VLEN && tclass != HDF5Constants.H5T_COMPOUND
    ...
    /**
     * Retrieves the object's metadata, such as attributes, from the file. Metadata, such as attributes, is stored in a
     * List.
     *
     ...
     *                       if the metadata can not be retrieved
     */
    public List<Attribute> getMetadata(int... attrPropList)
            throws HDF5Exception {
        try {
    ...
    /**
     * Writes a specific piece of metadata (such as an attribute) into the file. If an HDF(4&amp;5) attribute exists in
     * the file, this method updates its value. If the attribute does not exist in the file, it creates the attribute in
     * the file and attaches it to the object. It will fail to write a new attribute to the object where an attribute
     * with the same name already exists. To update the value of an existing attribute in the file, one needs to get the
     * instance of the attribute by getMetadata(), change its values, then use writeMetadata() to write the value.
     *
     * @param info
    ...
    @Override
    public void setFullname(String newPath, String newName)
            throws Exception {
    ...
    public static String descRegionDataset(long container, byte[] refarr) {
        ...
        try {
            ...
            new_obj_id = H5.H5Rdereference(container, HDF5Constants.H5P_DEFAULT, HDF5Constants.H5R_DATASET_REGION,
                    refarr);
    ...
    /**
     * Extracts compound information into flat structure. For example, compound datatype "nest" has {nest1{a, b, c}, d,
    ...
    public static void extractCompoundInfo(final H5Datatype dtype, String name, List<String> names,
        ...
        if (dtype.isArray()) {
            ...
            for (int i = 0; i < compoundMemberNames.size(); i++) {
                ...
                /*
                 * For ARRAY of COMPOUND and VLEN of COMPOUND types, we first add the top-level array or vlen type to
                 * the list of datatypes, and then follow that with a listing of the datatypes inside the nested
                 * compound.
                 */
                /*
                 * TODO: Don't flatten variable-length types until true variable-length support is implemented.
                 */
     ...
     *                       If there is an error at the HDF5 library level.
     */
    public long createCompoundFieldType(String memberName)
            throws HDF5Exception {
        ...
        try {
            if (this.isArray()) {
                ...
                if (sep >= 0) {
                    /*
                     * If a compound separator character is present in the supplied string, then there is an additional
                     * level of compound nesting. We will create a compound type to hold the nested compound type.
                     */
                ...
                if (memberIndex >= 0) {
                    ...
                    /*
                     * WARNING!!! This step is crucial. Without it, the compound type created might be larger than the
                     * size of the single datatype field we are inserting. Performing a read with a compound datatype of
                     * an incorrect size will corrupt JVM memory and cause strange behavior and crashes.
                     */
                    H5.H5Tpack(topTID);
                }
                else {
    ...
    private boolean datatypeClassIsAtomic(long tclass) {
        boolean retVal = !(datatypeClassIsComplex(tclass)
                | datatypeClassIsReference(tclass));

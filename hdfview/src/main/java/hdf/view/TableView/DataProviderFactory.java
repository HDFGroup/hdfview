...
public class DataProviderFactory {
     ...
     */
    public static HDFDataProvider getDataProvider(final DataFormat dataObject, final Object dataBuf,
    ...
    public static class HDFDataProvider implements IDataProvider {
            ...
            throws Exception
        {
            ...
            isNaturalOrder   = ((rank == 1) || (dataFormatReference.getSelectedIndex()[0] <
                                              dataFormatReference.getSelectedIndex()[1]));
            ...
            isContainerType = (this instanceof CompoundDataProvider || this instanceof ArrayDataProvider ||
        ...
        public int physicalLocationToBufIndex(int rowIndex, int columnIndex)
        {
            ...
            if (rank > 1) {
                log.trace(
                    "physicalLocationToBufIndex({}, {}): rank > 1; adjusting for multi-dimensional dataset",
        ...
        private void updateAtomicValue(Object bufObject, Object newValue, int bufIndex)
        {
            ...
            default:
                ...
                case 'D':
                    ...
                    break;
                default:
                    log.trace("updateAtomicValue(): bufObject={} bufIndex={} newValue={}", bufObject,
                              bufIndex, newValue);
    ...
    private static class CompoundDataProvider extends HDFDataProvider {
        ...
        CompoundDataProvider(final Datatype dtype, final Object dataBuf, final boolean dataTransposed)
            throws Exception
        {
            ...
            for (int i = 0; i < baseTypeProviders.length; i++) {
                ...
                try {
                    baseTypeProviders[i] =
                        getDataProvider(localSelectedTypes.get(i), dataBuf, dataTransposed);
                }
                catch (Exception ex) {
            ...
            if (relCmpdStartIndexMap.size() == 0) {
                ...
                throw new Exception(
                    "CompoundDataProvider: invalid compound field start index mapping of size 0 built");
            }
        ...
        public Object getDataValue(int columnIndex, int rowIndex)
        {
            log.trace("CompoundDataProvider.getDataValue: Input: columnIndex={}, rowIndex={}", columnIndex,
                      rowIndex);
            ...
            try {
                ...
                log.trace("CompoundDataProvider.getDataValue: Initial: fieldIdx={}, rowIdx={}", fieldIdx,
                          rowIdx);
                log.trace("CompoundDataProvider.getDataValue: nSubColumns={}, selectedMemberTypes.length={}",
                          nSubColumns, selectedMemberTypes.length);
                ...
                if (nSubColumns > 1) { // multi-dimension compound dataset
                    ...
                    log.trace(
                        "CompoundDataProvider.getDataValue: Multi-dim adjusted: fieldIdx={}, rowIdx={}, realColIdx={}",
                        fieldIdx, rowIdx, realColIdx);
                }

                int providerIndex = baseProviderIndexMap.get(fieldIdx);
                log.trace(
                    "CompoundDataProvider.getDataValue: INDEX MAPPING READ: fieldIdx={} -> providerIndex={}",
                    fieldIdx, providerIndex);
                log.trace("CompoundDataProvider.getDataValue: baseProviderIndexMap: {}",
                          baseProviderIndexMap);
                ...
                log.trace("CompoundDataProvider.getDataValue: colValue retrieved from dataBuf[{}], type={}",
                          providerIndex, colValue.getClass().getSimpleName());
                ...
                log.trace("CompoundDataProvider.getDataValue: Base provider[{}]: type={}, isContainerType={}",
                          providerIndex, base.getClass().getSimpleName(), base.isContainerType);
                if (providerIndex < selectedMemberTypes.length) {
                    log.trace("CompoundDataProvider.getDataValue: Member type: {}",
                              selectedMemberTypes[providerIndex].getDescription());
                }
                ...
                if (base instanceof CompoundDataProvider)
                    ...
                    int adjustedColIndex = columnIndex - arrCompoundStartIdx;
                    log.trace(
                        "CompoundDataProvider.getDataValue: adjustedColIndex={}, columnIndex={}, arrCompoundStartIdx={}",
        ...
        public void setDataValue(int columnIndex, int rowIndex, Object newValue)
        {
            log.trace("CompoundDataProvider.setDataValue: columnIndex={}, rowIndex={}, newValue={}",
                      columnIndex, rowIndex, newValue);

            if ((newValue == null) || ((newValue = ((String)newValue).trim()) == null)) {
                log.debug(
                    "CompoundDataProvider.setDataValue({}, {})=({}): cell value not updated; new value is null",
            ...
            log.trace("CompoundDataProvider.setDataValue: Old value at ({}, {}): {}", columnIndex, rowIndex,
                      oldVal);
            if ((oldVal != null) && newValue.equals(oldVal.toString())) {
                log.debug(
                    "CompoundDataProvider.setDataValue({}, {})=({}): cell value not updated; new value same as old value",
                    rowIndex, columnIndex, newValue);
                return;
            }
            ...
            try {
                ...
                log.trace("CompoundDataProvider.setDataValue: Initial: fieldIdx={}, rowIdx={}", fieldIdx,
                          rowIdx);
                log.trace("CompoundDataProvider.setDataValue: nSubColumns={}, selectedMemberTypes.length={}",
                          nSubColumns, selectedMemberTypes.length);
                ...
                if (nSubColumns > 1) { // multi-dimension compound dataset
                    ...
                    rowIdx         = rowIndex * nSubColumns + realColIdx;
                    log.trace(
                        "CompoundDataProvider.setDataValue: Multi-dim adjusted: fieldIdx={}, rowIdx={}, realColIdx={}",
                        fieldIdx, rowIdx, realColIdx);
                }

                int providerIndex = baseProviderIndexMap.get(fieldIdx);
                log.trace("CompoundDataProvider.setDataValue: INDEX MAPPING: fieldIdx={} -> providerIndex={}",
                          fieldIdx, providerIndex);
                log.trace("CompoundDataProvider.setDataValue: baseProviderIndexMap: {}",
                          baseProviderIndexMap);

                Object colValue = ((List<?>)dataBuf).get(providerIndex);
                if (colValue == null) {
                    log.debug("CompoundDataProvider.setDataValue({}, {})=({}): colValue is null", rowIndex,
                              columnIndex, newValue);
                    return;
                }
                log.trace("CompoundDataProvider.setDataValue: colValue retrieved from dataBuf[{}], type={}",
                          providerIndex, colValue.getClass().getSimpleName());
                 ...
                 */
                HDFDataProvider base = baseTypeProviders[providerIndex];
                log.trace("CompoundDataProvider.setDataValue: Base provider[{}]: type={}, isContainerType={}",
                          providerIndex, base.getClass().getSimpleName(), base.isContainerType);
                ...
                if (base.isContainerType) {
                    ...
                    log.trace(
                        "CompoundDataProvider.setDataValue: Container type: adjustedFieldIdx={} (fieldIdx {} - startIdx {})",
                        adjustedFieldIdx, fieldIdx, relCmpdStartIndexMap.get(fieldIdx));
                    log.trace(
                        "CompoundDataProvider.setDataValue: Calling base.setDataValue({}, {}, colValue, {})",
                        adjustedFieldIdx, rowIdx, newValue);
                    base.setDataValue(adjustedFieldIdx, rowIdx, colValue, newValue);
                }
                else {
                    log.trace(
                        "CompoundDataProvider.setDataValue: Non-container: Calling base.setDataValue({}, colValue, {})",
                        rowIdx, newValue);
            ...
            log.trace("CompoundDataProvider.setDataValue({}, {})=({}): finish", rowIndex, columnIndex,
                      newValue);
        ...
        {
            throw new UnsupportedOperationException(
                "setDataValue(int, Object, Object) should not be called for CompoundDataProviders");
        }
    ...
    private static class ArrayDataProvider extends HDFDataProvider {
        ...
        private Object[] retrieveArrayOfComplexElements(Object objBuf, int columnIndex, int rowStartIdx)
        {
            ...
            log.trace(
                "retrieveArrayOfComplexElements(obj={}, rowIndex={}, columnIndex={}) arraySize={}: start",
                objBuf, rowStartIdx, columnIndex, arraySize);
        ...
        private Object[] retrieveArrayOfAtomicElements(Object objBuf, int rowStartIdx)
        {
            ...
            log.trace("retrieveArrayOfAtomicElements(obj={}, rowIndex={}) arraySize={}: start", objBuf,
                      rowStartIdx, arraySize);
        ...
        {
            throw new UnsupportedOperationException(
                "getDataValue(Object, int) should not be called for ArrayDataProviders");
        }
        ...
        private void updateArrayOfCompoundElements(StringTokenizer tokenizer, Object curBuf, int columnIndex,
                                                   int bufStartIndex)
        {
            for (int i = 0; i < arraySize; i++) {
        ...
        private void updateArrayOfArrayElements(StringTokenizer tokenizer, Object curBuf, int columnIndex,
                                                int bufStartIndex)
        {
            for (int i = 0; i < arraySize; i++) {
                /*
    ...
    private static class VlenDataProvider extends HDFDataProvider {
        ...
        VlenDataProvider(final Datatype dtype, final Object dataBuf, final boolean dataTransposed)
            throws Exception
        {
            super(dtype, dataBuf, dataTransposed);
        ...
        {
            throw new UnsupportedOperationException(
                "setDataValue(int, Object, Object) should not be called for VlenDataProviders");
        }
    ...
    private static class StringDataProvider extends HDFDataProvider {
        ...
        StringDataProvider(final Datatype dtype, final Object dataBuf, final boolean dataTransposed)
            throws Exception
        {
            super(dtype, dataBuf, dataTransposed);
        ...
        public void setDataValue(int index, Object bufObject, Object newValue)
        {
            try {
                ...
                log.debug("setDataValue({}, {}, {}): cell value update failure: ", index, bufObject, newValue,
                          ex);
            }
            log.trace("setDataValue({}, {}, {}): finish", index, bufObject, newValue);
        }
    ...
    private static class CharDataProvider extends HDFDataProvider {
        ...
        CharDataProvider(final Datatype dtype, final Object dataBuf, final boolean dataTransposed)
            throws Exception
        {
            super(dtype, dataBuf, dataTransposed);
        }
    ...
    private static class EnumDataProvider extends HDFDataProvider {
        ...
        EnumDataProvider(final Datatype dtype, final Object dataBuf, final boolean dataTransposed)
            throws Exception
        {
            super(dtype, dataBuf, dataTransposed);
        }
    ...
    private static class BitfieldDataProvider extends HDFDataProvider {
        ...
        BitfieldDataProvider(final Datatype dtype, final Object dataBuf, final boolean dataTransposed)
            throws Exception
        {
            super(dtype, dataBuf, dataTransposed);
    ...
    private static class RefDataProvider extends HDFDataProvider {
        ...
        RefDataProvider(final Datatype dtype, final Object dataBuf, final boolean dataTransposed)
            throws Exception
        {
            super(dtype, dataBuf, dataTransposed);

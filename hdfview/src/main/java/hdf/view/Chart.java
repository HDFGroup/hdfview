...
public class Chart extends Dialog {
    ...
    public Chart(Shell parent, String title, int style, double[][] data, double[] xData, double[] yRange)
    {
        ...
        if (style == HISTOGRAM) {
            isInteger = true;
            barColor  = new Color(Display.getDefault(), Display.getDefault().getSystemColor(SWT.COLOR_LIST_SELECTION).getRGB());
        }
        else {
            isInteger = false;
        }
     ...
     */
    public void setLineColors(int[] c) { lineColors = c; }
                ...
                public void paintControl(PaintEvent e)
                {
                    ...
                    for (int i = 0; i <= xnpoints; i++) {
                        ...
                        if (gtOne) {
                            ...
                            g.drawString(value, (int)xp - (numberSize.x / 2),
                                         canvasBounds.height - numberSize.y);
                        }
                        else {
